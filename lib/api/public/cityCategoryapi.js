...
export function buildCategorySidebarSections(sidebarData = {}, { citySlug = '', cityName = '', activeCategorySlug = '' } = {}) {
    ...
    const streetItem = {
        categoryName: contextLabel ? `${contextLabel} Hotels by Streets` : 'Hotels by Streets',
        href: '#'
    };

    if (brands.length > 0 && safeCityPath) {
        const brandListingHref = `${safeCityPath}/${encodePathSegment(`${normalizedCitySlug}-hotels-by-brand.htm`)}`;
