...
export function normalizeCategoryResponse(response) {
    ...
    return {
        categories,
        hotels,
        locationInfo: payload.locationInfo ?? payload.location ?? {},
...
export function buildCategorySidebarSections(sidebarData = {}, { citySlug = '', cityName = '', activeCategorySlug = '' } = {}) {
    ...
    const contextLabel = String(cityName || formatCityName(normalizedCitySlug)).trim();
    const safeCityPath = normalizedCitySlug ? `/${encodePathSegment(normalizedCitySlug)}` : '';
    ...
    const streetListingHref = safeCityPath
        ? `${safeCityPath}/${encodePathSegment(`${normalizedCitySlug}-hotels-by-street.htm`)}`
        : '#';

    const brandListingHref = safeCityPath
        ? `${safeCityPath}/${encodePathSegment(`${normalizedCitySlug}-hotels-by-brand.htm`)}`
        : '#';
    const brandSectionTitle = contextLabel ? `${contextLabel} Hotels by Brands` : 'Hotels by Brands';
    const streetSectionTitle = contextLabel ? `${contextLabel} Hotels by Streets` : 'Hotels by Streets';

    const cityMoreItems = [];

    if (brands.length > 0 && safeCityPath) {
        if (brands.length <= 10) {
            mapped.push({
                sectionId: 'brands',
                title: brandSectionTitle,
                items: brands.slice(0, 10).map((brand) => {
                    ...
                    return {
                        ...
                        href: brandSlug ? `${safeCityPath}/${encodePathSegment(brandSlug)}` : brandListingHref
                    };
                }),
                maxVisible: 5
            });
            ...
            cityMoreItems.push({
                categoryName: brandSectionTitle,
                href: brandListingHref
            });
        }
    }

    if (landmarks.length > 0 && safeCityPath) {
        if (landmarks.length <= 10) {
            mapped.push({
                sectionId: 'landmarks',
                title: streetSectionTitle,
                items: landmarks.slice(0, 10).map((landmark) => ({
                    ...landmark,
                    categoryName: String(landmark?.landmarkName || '').trim(),
                    href: String(landmark?.landmarkUrl || '#')
                })),
                maxVisible: 5
            });
        } else {
            cityMoreItems.push({
                categoryName: streetSectionTitle,
                href: streetListingHref
            });
        }
    }

    if (cityMoreItems.length > 0) {
        mapped.push({
            sectionId: 'city-more',
            title: '',
            items: cityMoreItems,
...
export function buildRegionCategorySidebarSections(sidebarData = {}, { regionSlug = '', regionName = '', activeCategorySlug = '' } = {}) {
    const sections = buildSidebarSectionsForRegion(sidebarData, regionSlug, regionName);
    ...
    try {
        ...
        if (entityType === 'city' && cityId !== null && cityId !== undefined && cityId !== '') {
            resolvedContext = {
                ...
                cityName:
                    String(data?.entityName || data?.name || formatCityName(normalizedCitySlug)).trim() ||
    ...
    try {
        for (const path of candidatePaths) {
            ...
            if (data?.entityType === 'Region' && data?.entityId) {
                ...
                const resolvedCountryName = getFirstDefined(data?.countryName, data?.country?.name);
                return {
...
export async function resolveCategoryFromRegionSlug(categorySlug = '', regionSlug = '', countrySlug = '') {
    ...
    if (exactMatch) {
        return {
            ...
            countrySlug: regionContext.countrySlug || ''
        };
    }
